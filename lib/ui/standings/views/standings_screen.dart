  ...
  Widget build(BuildContext context) {
    ...
    return ChangeNotifierProvider(
      create: (context) => StandingsViewModel(
        standingRepository: context.read(),
      ...
      body: Consumer<StandingsViewModel>(
        builder: (context, viewModel, child) {
          if (viewModel.state == StandingsState.loading) {
            ...
            return Center(
              child: Text(viewModel.errorMessage ?? 'Erro ao carregar classificação'),
            );
          } else if (viewModel.standings.isEmpty) {
            return const Center(child: Text('Nenhuma classificação disponível.'));
          }
                ...
                Container(
                  ...
                  child: Row(
                    children: const [
                      ...
                      Expanded(child: Center(child: Text('V', style: TextStyle(fontWeight: FontWeight.bold)))),
                      Expanded(child: Center(child: Text('D', style: TextStyle(fontWeight: FontWeight.bold)))),
                      Expanded(child: Center(child: Text('Pts', style: TextStyle(fontWeight: FontWeight.bold)))),
                    ],
                  ),
                ),
                // List
                      ...
                      return Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(child: Center(child: Text('${standing.wins}'))),
                            Expanded(child: Center(child: Text('${standing.losses}'))),
                            Expanded(child: Center(child: Text('${standing.points}'))),
                          ],
                        ),
                      );
                    },
